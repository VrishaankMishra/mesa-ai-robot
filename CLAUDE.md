...
- **Two repos — private first, always.** `VrishaankMishra/mesa-ai-robot-private` is the working
  repo: every branch, commit, push and PR goes there. The public `VrishaankMishra/mesa-ai-robot`
  is a *generated* copy — `scripts/publish_public_copy.sh` rebuilds it from private `main` with
  `EXCLUDE_PATHS` scrubbed from the entire history, then force-pushes. **Never push, branch or
...
- **What never goes public** (see `private/README.md`): anything that names a person we might
  ...
  and commit messages are not scrubbed by the publish script — keep them to mechanism.
